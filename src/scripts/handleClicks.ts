			...
			if (confirmationScreen == document.querySelector('.import-create')) {
				...
				if (confirm) {
					let input = confirmationScreen
						...
						.replaceAll('\n', '')
						.replaceAll('-', '')
					let isValid = /[g-z~`!#$%\^&*+=\[\]\\';,/{}|\\":<>\?]/g.test(input) ? false : input.length == 0 ? false : true
					if (isValid) isValid = input.length % 6 == 0 ? true : false
